...
function gapStatus(inj){
  ...
  const cb = document.body.classList.contains('color-blind-mode');
  if(gaps.length===0) return { status:'complete', color:cb?'#555':'var(--mild)', bg:cb?'#f0f0f0':'#f0fdf4', border:cb?'#bbb':'#bbf7d0', label:'Complete', gaps };
  return { status:'incomplete', color:cb?'#444':'var(--moderate)', bg:cb?'#e8e8e8':'#fffbeb', border:cb?'#999':'#fde68a', label:'Needs Evidence ('+gaps.length+')', gaps };
}
