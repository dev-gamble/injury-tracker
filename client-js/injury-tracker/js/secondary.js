...
// Helper: remove a condition's individual pin from the map
function _removeCondPinIfExists(condId){
...
const COMMON_SECONDARIES = {
  ...
  'Hypertension': [
    ...
    {name:'Erectile dysfunction', dc:'DC 7522'},
    {name:'Vision impairment', dc:'DC 6066'},
    {name:'Coronary artery disease', dc:'DC 7005'},
    {name:'Cerebrovascular accident (stroke)', dc:'DC 8007'},
  ],
...
// ── SECONDARY LOCATION PROMPT ──
// When adding a secondary from suggestion chips, ask side only for sided conditions

// Conditions that are NEVER sided — mental health, head/brain, organ/systemic
const _NON_SIDED_KEYWORDS = [
  // Mental health
  'ptsd','posttraumatic','depression','depressive','anxiety','generalized anxiety',
  'panic','bipolar','dysthymia','insomnia','sleep','adjustment','obsessive',
  'substance','eating disorder','somatic','schizophrenia','psychotic',
  // Head / brain / sensory
  'migraine','headache','tbi','traumatic brain','tinnitus','hearing','vision',
  'vertigo','sinusitis','rhinitis','deviated septum','sleep apnea',
  'cognitive','concussion','tmj','temporomandibular',
  // Organ / systemic / midline
  'gerd','ibs','irritable bowel','hiatal hernia','gastritis','ulcer',
  'diabetes','hypertension','heart','cardiac','arrhythmia',
  'asthma','copd','chronic obstructive','restrictive lung',
  'fibromyalgia','chronic fatigue','erectile','hypothyroid','hyperthyroid',
  'liver','kidney','crohn','colitis','diverticulitis',
  'obesity','anemia','lupus',
  // Skin (typically rated globally, not per side)
  'eczema','dermatitis','psoriasis','acne','hives','urticaria',
  // Spine / back / neck (midline)
  'lumbar','cervical','thoracic','spinal','spine','stenosis','disc',
  'degenerative','radiculopathy','sciatica','spondylosis','scoliosis',
  'intervertebral','back strain','neck strain',
];

function _isNonSidedCondition(name){
  const lower = name.toLowerCase();
  return _NON_SIDED_KEYWORDS.some(kw => lower.includes(kw));
}

function addSecWithLocationPrompt(claimId, name){
  if(!name) return;

  // Skip side prompt for non-sided conditions — go straight to add
  if(_isNonSidedCondition(name)){
    if(!_secAddState[claimId]) _secAddState[claimId] = {};
    _secAddState[claimId].secSide = 'na';
    addSecFromDropdown(claimId, name);
    return;
  }

  // Build side selection popup for sided conditions only
  let el = document.getElementById('sec-location-prompt');
  if(el) el.remove();
  el = document.createElement('div');
  el.id = 'sec-location-prompt';
  el.className = 'bilateral-overlay';

  const escaped = name.replace(/'/g,"\\'").replace(/"/g,'&quot;');

  el.innerHTML =
    '<div class="bilateral-dialog" style="max-width:340px;">' +
      '<div class="bilateral-dialog-title" style="font-size:14px;">Which side is affected?</div>' +
      '<div class="bilateral-dialog-cond">' + name + '</div>' +
      '<div style="display:flex;flex-direction:column;gap:8px;margin-top:14px;">' +
        '<button class="bilateral-btn bilateral-btn-no" style="flex:1;" onclick="_confirmSecSide(\''+claimId+'\',\''+escaped+'\',\'left\')">Left</button>' +
        '<button class="bilateral-btn bilateral-btn-no" style="flex:1;" onclick="_confirmSecSide(\''+claimId+'\',\''+escaped+'\',\'right\')">Right</button>' +
        '<button class="bilateral-btn bilateral-btn-yes" style="flex:1;" onclick="_confirmSecSide(\''+claimId+'\',\''+escaped+'\',\'both\')">Both (Bilateral)</button>' +
      '</div>' +
      '<div style="margin-top:10px;text-align:center;">' +
        '<button style="background:none;border:none;color:var(--muted);font-size:11px;cursor:pointer;text-decoration:underline;" onclick="document.getElementById(\'sec-location-prompt\').remove()">Cancel</button>' +
      '</div>' +
    '</div>';
  document.body.appendChild(el);
}

function _confirmSecSide(claimId, name, side){
  const el = document.getElementById('sec-location-prompt');
  if(el) el.remove();
  if(!_secAddState[claimId]) _secAddState[claimId] = {};
  _secAddState[claimId].secSide = side;
  addSecFromDropdown(claimId, name);
}

// Old area/side helpers removed — replaced by _confirmSecSide
...
function renderSecondaryAddDropdowns(claimId, group){
  ...
  // Condition dropdown — separate physical from mental health secondaries
  if(area){
    let condList = [];
    ...
    profileDomains.forEach(domain => {
      ...
      domain.levels.forEach(lv => {
        ...
        h += '<button class="cr-hd-btn' + (isActive ? ' cr-hd-active' : '') + '"' +
          ...
          '<span class="cr-hd-body">' +
            '<span class="cr-hd-lbl">' + lv.label + '</span>' +
            (lv.description ? '<div class="cr-hd-desc">' + lv.description + '</div>' : '') +
          '</span>' +
        '</button>';
      });
      h += '</div>';
